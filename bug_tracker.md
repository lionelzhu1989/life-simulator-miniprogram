...
## Bug #011: 页面完全空白（JS语法错误）
- **发现时间**: 2026-07-05 v4.4.9
- **症状**: 点击宠物后整个页面崩溃空白，无任何内容显示
- **根因**: v4.4.8修复商店按钮`onclick`时将`openPetStore()`替换为`switchPetTab('shop',document.getElementById('petSubTabs').querySelector('.pet-subtab'))`，但`'shop'`、`'petSubTabs'`、`'.pet-subtab'`三个字符串参数未使用反斜杠转义。在JS单引号字符串内部，未转义的单引号会提前终止外层字符串，导致`switchPetTab('shop'`后面出现`Unexpected identifier 'shop'`的JS语法错误。整个`<script>`块无法解析，所有JS功能失效，页面完全空白
- **修正**: 为三个字符串参数添加反斜杠转义：`'shop'` → `\'shop\'`、`'petSubTabs'` → `\'petSubTabs\'`、`'.pet-subtab'` → `\'.pet-subtab\'`
- **预防措施**: 
  1. **JS字符串铁律**：在单引号包裹的JS字符串内部嵌套单引号字符串参数时，内层所有单引号必须转义为`\'`
  2. **语法检查强制化**：所有HTML/JS修改后用`node --check`检查被修改的`<script>`块（无法直接check HTML则先提取脚本为临时.js文件后检查）
  3. **字符串拼接审查**：含`\"`和`\'`混用的`h+=`拼接行，重点检查每个`'`是否出现在正确的嵌套层级
  4. **模板字符串优先**：未来建议使用反引号模板字符串`` `...${var}...` ``替代单引号拼接，避免转义地狱

---

*最后更新: 2026-07-05 22:52*